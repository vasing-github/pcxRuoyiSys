...
function createExampleTownMap(option2, tooltipOption2) {
    var myChart_map2 = echarts.init(document.getElementById('bjn2'));
    myChart_map2.setOption(option2);
    tools.loopShowTooltip(myChart_map2, option2, tooltipOption2);
}

function intTownMap() {
    var postData = {};
    var postUrl = getPostUrl("zwry-town");
        ...
        if (!!result && result.code == 200 && !!result.data) {
            $.each(result.data, function (i, item) {
                ...
                if (keyarry.length > 0 && valarry.length > 0) {
                    ...
                    if (builderJson.yAxis.length < 10) {
                        builderJson.yAxis.push(strcut(keyarry[0], 6, "...") );
                        builderJson.xAxis.push(valarry[0]);
                    }
                }
            })
        }
        builderJson.yAxis = builderJson.yAxis.reverse();
        builderJson.xAxis = builderJson.xAxis.reverse();
...
function loadTownMap(dataList) {
    var minNumber = 0;
    var maxNumber = 5;
    //var strData = "";//{ name: '安县', value: 10, cvalue: 16 }

    if (!!dataList && dataList.length > 0) {
        for (var i = 0; i < dataList.length; i++) {
            var cdata = dataList[i];
            //strData += ",{name:'" + cdata.name + "',value:" + cdata.value + "}";
    ...
    //}
    //strData = "[" + strData + "]";
    var mapdata = dataList;// eval(strData);

    $.get('json/511923.json?v=20201021', function (areaJson) {
        echarts.registerMap('TownMap', areaJson);
        var optionTown = {
            tooltip: {
                trigger: 'item',
                //formatter: '总共办理{c}件'
                ...
                formatter: function (params) {
                    var rHtml = "";
                    if (!!params.data) {
                        rHtml = params.data.name + '：<br />在外人员' + params.data.value + '人';//，已返回' + params.data.bvalue + '人
                    } else {
                        rHtml = params.data.name + '：<br />在外人员0人';
                    }
                    return rHtml;
                },
                textStyle: {
            ...
            series: [
                {
                    ...
                    label: {                                     //图形上的文本标签，可用于说明图形的一些数据信息，比如值，名称等
                        normal: {
                            ...
                            textStyle: mytextStyleTown,              //普通状态下的标签文本样式。
                        },
                        emphasis: {
        ...
        createExampleTownMap(optionTown, {
            loopSeries: true
        });
    });
}

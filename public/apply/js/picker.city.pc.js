            ...
            "sub": [
                {
                    ...
                    "name": "经开区",
                    "sub": [
                        { "code": "110101", "name": "福星社区" },
                        { "code": "110102", "name": "双星社区" },
                        { "code": "110103", "name": "星光社区" },
                        { "code": "110104", "name": "太平社区" },
                        { "code": "110105", "name": "小桥街社区" }
                    ]
                },
                {
                    "code": "1101",
...
+ function ($) {
    ...
    $.fn.cityPCPicker = function (params) {
        var getCities = function (d) {
            for (var i = 0; i < pcraw.length; i++) {
                if (pcraw[i].code === d || pcraw[i].name === d) return sub(pcraw[i]);
            }
            return [];
        };
        ...
        var getDistricts = function (p, c) {
            for (var i = 0; i < pcraw.length; i++) {
                if (pcraw[i].code === p || pcraw[i].name === p) {
                    for (var j = 0; j < pcraw[i].sub.length; j++) {
                        if (pcraw[i].sub[j].code === c || pcraw[i].sub[j].name === c) {
                            return sub(pcraw[i].sub[j]);
                        }
                    }
                }
            }
        ...
        return this.each(function () {
            ...
            var provincesName = pcraw.map(function (d) {
                return d.name;
            });
            var provincesCode = pcraw.map(function (d) {
                return d.code;
            });
            var initCities = sub(pcraw[0]);
            var initCitiesName = initCities.map(function (c) {
                return c.name;
            });
            var initCitiesCode = initCities.map(function (c) {
            ...
            var initDistricts = sub(pcraw[0].sub[0]);
